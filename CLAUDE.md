...
At session start, right after reading MEMORY.md: if `.claude/restart-handoff.md` exists, CLAIM it before reading it, in exactly this sequence: `mkdir -p .claude/restart-journal && mv .claude/restart-handoff.md .claude/restart-journal/<yyyy-MM-dd-HHmmss>.md` (timestamp = time of consumption, local, 24h; if the target name exists, increment a numeric suffix — `-2`, `-3`, ... — until an unused name is found, never overwriting an existing journal entry), then verify `.claude/restart-handoff.md` no longer exists. Only after it is confirmed gone: read the journal file you just created and act on it — run its verification checks, re-establish its watch list (then poll each watched agent's current state once — events during the restart were lost), and resume its work. A crash between claim and read loses only the trigger — the content is safe in the journal for Kyle to point at. If the move fails for any reason, fall back to read-then-delete: read the handoff, DELETE it, and only then act — deletion beats replay; if deletion also fails, stop and tell Kyle before acting on the handoff.
