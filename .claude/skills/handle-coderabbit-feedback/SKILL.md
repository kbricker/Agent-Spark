...
2. **Distinguish actionable from informational, with Jev's triage first (plan 1048.2; on a `jevLane` agent).** Write the round's CR findings as a JSON array in the finding shape — `{ "lens": "coderabbit", "severity": <CR's, or "minor" for a nit>, "file", "line", "summary": <the comment's first sentence>, "failure_scenario": <the comment body> }` — and run the same three commands step 5.5 of `fast-track-plan` names (state builder, `--preset finding-triage`, report), passing as `--prior-skipped` the findings you declined in earlier CR rounds and in the internal pass. `SKIPPED` findings are the pushbacks you already made — read each against the skipped finding it matches and confirm they are one defect, check whether the fixes since then changed its premise (a guard removed, a caller added), and if it is the same defect on the same premise, one batched comment, not a re-argument; `DUP` findings fold under the finding they repeat once you have read both and agree they are one defect; the rest you read worst severity first. Focus on actionable findings and nits you decide to absorb. Apply `manage-scope-creep` sift to anything adjacent-but-outside the diff.
3. **Fix or push back.**
