...
- **Review split is non-negotiable (Kyle, 2026-09-01): Grok implements, Claude reviews.** Grok output gets the same step 5.5 adversarial pass as any dev work — Grok never reviews, never commits, never pushes. The Codex review lane in step 5.5 does not bend this: a `codexLane` agent may ADD a Codex pass over Grok's output, but Claude's own lenses still run and still adjudicate.
...
- **Codex review lane — an OPTION for `codexLane` agents (plans #1014/1014.1), never a substitute.** Agents flagged `codexLane: true` in `composition.json` (overwatch, vaexdev, vaexdev2 today) may run one more lens on the same staged diff: `node C:/Projects/wfa2/hooks/codex-dispatch.mjs --cwd <repo> (--base <default> | --staged | --commit <sha>) --plan <bare id> --for-session <your session guid>` — headless OpenAI Codex on the ChatGPT subscription, the seven-lens brief in `hooks/codex-review-brief.md`, `-s read-only`, schema'd findings back on stdout. Claude's multi-lens subagent pass above stays the default AND stays mandatory; Codex's findings are adjudicated by you like any subagent's and logged with reviewer `codex-lane`. Measured 2026-09-06 on VaEx PR #430: Codex raised 2 findings (one false) against Claude's 11 with zero overlap — a Codex-only pass would have shipped both Majors, so it widens the net and never replaces it. `ok:false` from the dispatcher is a FAILED run (auth, no output, a lens not covered), not a clean review — an empty findings list is clean only when `ok` is true. Not flagged means the lane is not yours. An agent whose repos have no CodeRabbit (spark: Orbital, TendWright) is not flagged today and is flagged only with Kyle's explicit yes — there the 5.5 pass is the only review, so a Codex lens there must stay additive to Claude's pass, never stand in for it. Rebuild the benchmark case before trusting a model claim: `node C:/Projects/wfa2/tests/codex-bench/bench.mjs`.
