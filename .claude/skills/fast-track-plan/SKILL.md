...
description: The orchestration workflow, used on every plan. On a lane-flagged agent you orchestrate only — Grok implements (step 4), astra reviews (step 5.5), you shape, brief, adjudicate, commit, push, merge and deploy; elsewhere you play dev + review inline and fan out to Agent/Task subagents for parallelizable work. Walks plan status through the gated state machine, runs the mandatory internal adversarial review, merges and deploys. There is no second pipeline — work too large for one context gets decomposed across subagents.
scope: global
---

# Fast-track Plan Workflow (default path)

This is the orchestration workflow. On a lane-flagged agent you orchestrate only — `grokLane` in composition.json means Grok implements (step 4), `codexLane` means astra reviews (step 5.5); the same three agents carry both today, and each flag is read on its own step — you shape, brief, adjudicate, commit, push, merge and deploy (Kyle 2026-09-25, plan #1046); elsewhere you play dev + review inline and fan out to Agent/Task subagents as needed. **There is no second path** — the separate-process pipeline that used to serve as an escape hatch was retired on 2026-08-16.
...
- **Genuinely large — many files, many sub-areas.** Split it across subagents by sub-area and synthesise their reports yourself. You keep the design decisions; they do the lookup and the mechanical work — on a `grokLane` agent the mechanical implementation per sub-area goes to Grok serially, or the ticket forks into sub-plans with one worktree each (step 4).
...
**Grok dev lane — the DEFAULT implementation path on lane-flagged agents (plans #984/984.1; Grok-by-default is Kyle's 2026-09-25 call, plan #1046; flag-gated: agents with `grokLane: true` on their composition.json entry — the flag is the rule, a roster in prose goes stale).** One machine-level grok install + Kyle's SuperGrok login serves every flagged primary on this box; the flag on your composition.json entry is the whole gate (the dispatcher allow, when Kyle adds it, lives in the shared hive profile settings.json — the auto-mode classifier never reads a workspace .claude/settings.json, plan #1046) — not flagged means this lane is not yours; implementation goes to your dev subagent as before. On a flagged agent you write no code: every code change on any branch you push — the PR flow and the no-PR content-branch flow alike, first implementation and review fixes alike, a one-line fix included — goes to Grok (flat subscription fee — zero Claude context/quota consumed; Kyle 2026-09-25: Fable/Opus orchestrate only). What stays with you is orchestration output: briefs, plan records, and the whole config-repo flow — canonical rule text under `orchestrator-shared/` and workspace-local config alike — which commits to the default branch directly, has no worktree-to-main recipe, and gets its review from step 5.5. Everything else kept off Grok is an exception, and exceptions are logged; the lane-choice bullet below names them. On a Grok piece, skip the "create a feature branch" bullet above and do not check the branch out in your clone — `worktree add -b` creates the branch and owns it, and a branch that already exists, or is checked out elsewhere, makes the command fail:

```
git -C <repo> fetch origin && git -C <repo> worktree add -b plan<id>/<slug> C:/Projects/grok-worktrees/<repo-name>-<id> origin/<default>
node C:/Projects/wfa2/hooks/grok-dispatch.mjs --cwd C:/Projects/grok-worktrees/<repo-name>-<id> --plan <id> --for-session <your-session-guid> --brief <brief-file>
```

`--plan` takes the BARE numeric id (a display number like 984.1 is rejected loudly). `--timeout <sec>` bounds the whole dispatch, nudge included, and defaults to 3600 — pass a shorter one for a small piece so a hang fails fast instead of eating the hour. `--for-session` is the dispatching session's GUID — the directory name in your scratchpad path — and it is what files the dispatch as a subagent row under YOUR agent's accounting (visible on your next turn), exactly like a Claude subagent; without it the work still accounts to the `grok` lane row and the ticket, just not under you.

**One worktree per ticket** — that's what makes 2-3 concurrent grok dispatches on separate tickets safe (the session store is parallel-safe; shared working trees are not). A worktree checks out only tracked files, so local secret files (RemoteAgent appsettings.local.json etc.) never enter grok's working tree (finding 1398 — residual: grok can still read absolute paths anywhere if told to; keep briefs clean). After the dispatch, YOU commit Grok's edits on the worktree branch (Grok never commits); after review, YOU push from the worktree, then `git worktree remove` it. A worktree of some repos cannot build — VaEx's `*.csproj`/`*.sln` are gitignored and there is no `Library/` (vaexdev, plan 1000.3) — so brief Grok compile-blind there (no build step in the brief, no claim of a green build accepted), and after that commit run the headless build in a checkout that can build — for VaEx a separate clone, the shared VaEx4 tree — on that exact commit, fetched over the local path so nothing is pushed before review: `git -C <build-clone> fetch <repo> plan<id>/<slug> && git -C <build-clone> checkout --detach FETCH_HEAD` (one recipe for a separate clone and for a worktree of the same repo alike; git refuses to check the branch itself out twice, a detached checkout is allowed). The build checkout must hold no edits of its own — `git status --porcelain` shows nothing beyond what Unity regenerates on open; anything a person or an earlier ticket left there makes the build lie. If the only buildable checkout carries drift, `git stash -u` it, build detached, drop what the build itself rewrote or generated (`git checkout -- . && git clean -fd` there — Unity regenerates tracked settings files and untracked .meta files, a dirty detached tree makes the switch back refuse, and a regenerated untracked file blocks the pop; the user's own untracked files are safe in the stash), then `git checkout -` back to where it was and only there `git stash pop --index` (`--index` puts a staged/unstaged split back as it was) — popping onto the plan commit strands the drift or conflicts with it; if the checkout is mid-flight on another ticket, that is exit (1) for this piece. The build is done before the step 5.5 pass, because a build of any checkout that does not hold Grok's commit is a build of the old code. Review fixes are Grok's too — a follow-up dispatch with the same `--plan` resumes its session with full context — committed by you in the worktree (the branch), never in the detached build checkout; re-detach that checkout onto the new commit to rebuild after each round, restoring it (`checkout -` and the pop) after every build and before the next detach — a second detach without a restore in between makes `checkout -` land on the previous build commit, not on the branch you started from. The build checkout is single-flight: never detach it onto another commit while a build is running in it, so two concurrent tickets take turns on it or get a build checkout each.

Run it in the background; the JSON result carries the schema'd report (`summary/changed_files/tests/blockers`), `groundTruth` (git-verified actual changes vs claims, baselined against pre-dispatch dirt — treat `claimedMissing`/`unclaimed` mismatches and any `gitViolation` as findings), `toolUseCount` (984.2: grok's tool calls this dispatch, also filed on the subagent row), and per-(plan, cwd) session continuity — follow-up dispatches with the same `--plan` resume with full context (`--fresh` starts over; plan-less dispatches never resume). Since 984.2 an implementation dispatch that ends its first turn on intent alone (a known zero tool calls, no edit, empty `changed_files`) is nudged once in the same session and, if still empty, returned as `ok:false, stalledIntentOnly:true`. The verdict needs the tool count, which comes from grok's session store — when `toolUseCount` is null the dispatcher reports the run as it always did, so read `changed_files` yourself. A plan-scoped dispatch that is MEANT to answer without editing (a probe, a question) should pass `--no-nudge`.
...
- **The split is non-negotiable (Kyle 2026-09-25, superseding 2026-09-01's "Claude reviews"): Grok implements, astra reviews, Claude orchestrates.** Grok output gets the step 5.5 pass like any dev work — Grok never reviews, never commits, never pushes; astra never edits; you adjudicate astra's findings and send the fixes back to Grok.
- **Lane choice on a flagged agent is not a judgment call: Grok by default (Kyle 2026-09-25). The exits below are plan #1046's shaping, not Kyle's words.** A design still moving under the implementation is not an exit: iterate WITH Grok — follow-up dispatches with the same `--plan` resume its session with full context — never by writing the code yourself. Two named exits, each recorded as a `decision` on the plan's shaping log (`hive_plan_log_add`) before you take it, so an exception stays auditable and never becomes the habit: (1) **containment** — the piece needs something Grok's worktree or grok-dispatch cannot give it: work that must happen inside the Unity editor (scene or prefab authoring, an import, a play-mode check — a compile alone is the compile-blind case above, not this exit), hive MCP tools and every `HIVE_*` variable, a Hive channel, `gh`, git checkout/stash/fetch/pull/commit/push, process kills, a deploy step — so you or your dev subagent take it; (2) **lane failure** — the dispatch comes back `ok:false` (auth or quota, timeout, `stalledIntentOnly` after its nudge, `gitViolation`) twice on the same piece (before the retry, put the worktree back on the pre-dispatch sha with the discard command below unless the partial edits are exactly what it should continue from), or the lane is down for a reason you cannot fix in place (SuperGrok login expired, a Grok outage): your dev subagent takes the piece and the decision cites the envelope field; a failed dispatch may have left partial edits in the worktree, so inspect them and either brief the subagent to start from that state or discard them entirely, back to the commit you dispatched from (`git -C <worktree> checkout -f -B plan<id>/<slug> <pre-dispatch sha> && git -C <worktree> clean -fdx` — note that sha before dispatching; `-x` because a gitignored file the dispatch left behind would otherwise survive; `checkout -f -B` puts the plan branch back on it whatever a `gitViolation` left checked out, where a bare `reset --hard` would move whichever branch HEAD was on, and a plain `checkout -- .` leaves staged and untracked files behind) — say which in the decision. An exit-2 refusal with no envelope (GROK_HOME unset, a display number in `--plan`, a bad `--cwd`) is a caller error — fix the call; it is neither a failure to count nor a fallback to take. Parallel sub-areas of ONE ticket are not an exit either: Grok takes them serially in the same session (`--plan` continuity), or you fork the ticket into sub-plans with one worktree each. Every lane failure you exited on goes in your step 11 report, because the lane is Kyle's to fix and the report is where he learns it broke. Not flagged means Grok is not yours and your dev subagent stays the path.
...
Before the first push of any fast-track PR branch — the push that opens it to CodeRabbit, and regardless of whether you'll wait for CR — run an internal adversarial review on the complete outgoing diff (plan #650, Kyle 2026-07-24). **Who reviews (Kyle 2026-09-25, plan #1046): on a `codexLane` agent the Codex lane on `gpt-6-astra` IS this pass — you do not review, you orchestrate it: run the lane, adjudicate its findings, route the fixes to Grok, rerun until clean. An agent without the lane runs the Claude subagent pass described under its own heading below.** Stage everything you intend to ship first (explicit `git add <path>` per step 5), then give the reviewer `git diff origin/<default>...HEAD` (fetch first — local `<default>` trails after every merge, and a stale one puts upstream commits in the plan's diff) plus `git diff --cached`, run in the tree the work is in — on a Grok piece the worktree, named in the brief — and confirm via `git status` that no unstaged or untracked file you meant to include is missing from what it reviewed. No size exemption — trivial diffs still get the pass. Inline self-review alone does not hit the "rigor of a dedicated reviewer" bar — wfa2 PR #104 shipped a Major to CR (dedup filter silently dropping human review events, finding #112) that this pass is specifically built to catch.

- **Codex lane — THE review on `codexLane` agents (plans #1014/1014.1; the review since 2026-09-25, plan #1046).** Agents flagged `codexLane: true` in `composition.json` (the flag is the rule) run the pass as headless OpenAI Codex on the ChatGPT subscription — model `gpt-6-astra`, the seven-lens brief in `hooks/codex-review-brief.md`, `-s read-only`, schema'd findings back on stdout — over exactly the diff described above, every round. On any branch flow — a PR branch or a no-PR content branch alike — write `git diff origin/<default>...HEAD` followed by `git diff --cached`, then `git log origin/<default>..HEAD` so the commit messages ride along, to a file and hand it over with `--diff-file`, because the dispatcher's own selectors each see only half of a branch with fixes re-staged (`--base` is commits only, `--staged` is the index only); `--cwd` is the tree the diff came from — on a Grok piece the worktree, not your clone — because the dispatcher reads the touched files' full contents from there. In this mode it skips dot-segment paths (`.coderabbit.yaml`, an agent's `.claude/settings.json`), so read `diff.contextSkipped` in the envelope; a round that is commits-only and touches such files can run `--base origin/<default>` instead, which packs them. On the config-repo flow keep everything uncommitted until the pass is clean and run every round on `--staged` — re-staged fixes are in the index, and the selector keeps the context files `--diff-file` drops for dot-segment paths. If you committed early, `--commit HEAD` sees one commit and no staged fix, so the rerun is a `--diff-file` of `git diff <pre-change sha>..HEAD` followed by `git diff --cached`, the sha noted with `git rev-parse HEAD` before you started, because a workspace repo may have no `origin` — with the lost context files as the price of committing first. `node C:/Projects/wfa2/hooks/codex-dispatch.mjs --cwd <tree> (--diff-file <path> | --staged | --commit <sha>) --plan <bare id> --for-session <your session guid — the directory name in your scratchpad path> --model gpt-6-astra --timeout 900`. Pass the model explicitly every time: a bare dispatch runs whatever `DEFAULT_MODEL` the dispatcher pins, and that pin is not this skill's to assume (bench on CLI 0.157.0: js fixture 5/5, strict 5/5, 30 s, envelope in `tests/codex-bench/results/2026-09-25-astra-cli-0.157`). Adjudicate the envelope yourself: every finding carries a concrete failure scenario by the brief's contract — accept it only if the scenario holds against the code, consciously skip the rest, and read the observations as the escape valve for real-but-unprovable concerns. Valid findings go back to Grok as a follow-up dispatch on the same `--plan` (on the config-repo flow the fix is your own prose); re-stage the fixed paths and rerun the lane on the post-fix diff; loop until an envelope comes back with no valid findings — a round is not clean until its envelope is back and adjudicated. Codex has no round memory, so a finding you consciously skipped comes back unchanged next round — check only whether this round's fixes changed its premise (a guard removed, a caller added), then skip it again without re-litigating the rest. Failure exits: `ok:false` (auth, no output, `versionTooOld`, `modelUnavailable`, a lens not covered) is a FAILED run, not a clean review — an empty findings list is clean only when `ok` is true. Re-run a failed run once; a second failure in the same round takes the lane out for the rest of this pass — log that once on the plan, finish the pass on the Claude subagent pass below as the fallback, and put a `versionTooOld` or `modelUnavailable` in your step 11 report, because a retired model or a downgraded CLI is Kyle's call, not something to keep failing on every plan. An exit-2 refusal with no envelope (CODEX_HOME unset, an empty diff, a display number in `--plan`) is a caller error — fix the call; it is never a run to retry or to skip past. The one empty diff that is not an error is the pass ending: fixes that removed the whole change leave nothing to ship, and the pass ends there. Not flagged means the lane is not yours: an agent whose repos have no CodeRabbit (spark: Orbital, TendWright) is not flagged today and is flagged only with Kyle's explicit yes — there the Claude subagent pass is the only review. The 2026-09-06 comparison on VaEx PR #430 (Codex 2 findings, one false; Claude 11; zero overlap) is on record in plan 1046's shaping log; the single reviewer is Kyle's informed choice. Rebuild the benchmark case before trusting a model claim, naming the model — bare, the bench measures the dispatcher's pinned default: `node C:/Projects/wfa2/tests/codex-bench/bench.mjs --model gpt-6-astra`.
- **Claude subagent pass — for agents without the Codex lane, and the fallback when the lane is out for a pass.** The bullets that follow describe it; on a `codexLane` agent read them for the vocabulary of what a review must cover, not as steps to run.
...
- **Config-repo direct commits still get this pass.** Having no PR is a property of the *transport* — CodeRabbit is not wired to these repos — and not a review exemption. A skill, hook or memory that programs agent behaviour is code executed by an LLM rather than a CPU, and it reaches the whole fleet.
  - **This flow needs its own trigger and its own diff expression**, because the ones at the top of 5.5 assume a PR-opening push and silently produce nothing here. Trigger: **before `git push`**. Diff to hand the reviewer: `git diff --cached` before you commit, or `git show HEAD` / `git diff <pre-change sha>..HEAD` after (the sha from `git rev-parse HEAD` before you started — a workspace repo may have no `origin`). Do **not** use `git diff <default>...HEAD` on this flow — HEAD *is* the default branch, so it is empty by construction and the pass will self-certify having reviewed nothing.
  - Author the change **canonically** in `orchestrator-shared/` (committed direct to `main` — rule changes never get a PR, Kyle 2026-08-11), never by editing a **propagated copy** — the next sync reverts it silently, and it dodges review without anyone consciously deciding to. Genuinely workspace-local config is a different thing and is legitimate: a workspace `CLAUDE.md`, `.claude/settings.json`, `.mcp.json`, local-only memories, **and workspace-local skills and hooks** are authored locally. Local-only skills are the majority case, not an edge case (overwatch alone holds about ten with no `orchestrator-shared` counterpart: `deploy-hive`, `restart-remoteagent`, `propagate-shared-config` and others). Filing one of those canonically is the opposite failure: it propagates VM names and deploy keys to agents that must never run them.
...
> **STOP — gate on step 5.5.** If you are about to run `gh pr create` (or the first `git push` of the branch) and have **not** completed the step 5.5 review loop (the Codex lane; or the Claude subagent pass where the lane is not yours, or where the lane failed twice in a round and was taken out for this pass) to a clean round, go back and do it now. Not "run it after and fix what it finds" — before.
>
...
>
> Self-check, three questions: did the reviewer read the full outgoing diff? did the final round come back with nothing new? are the survivors logged? If any answer is no, you are not at step 6 yet.
...
- If the plan touches anything `deploy-hive.ps1` ships or that runs beside it — `AgentStudio2/`, `McpBridge/src/`, `RemoteAgent/`, `hooks/`, `agent-templates/`, `orchestrator-shared/composition.json`, `vm-config/` — invoke `/deploy-hive` as a sub-step.
...
One or two sentences. What shipped, where to look, follow-ups. Mention any side findings and offer to file separate tickets. Name any lane exit you took — a Grok lane failure, a Codex lane taken out of a pass, a `versionTooOld` or `modelUnavailable` — the lanes are Kyle's to fix, and this report is where he learns they broke.

## Fan-out pattern (Agent/Task subagents)

Fast-track is inline by default for orchestration work — on a lane-flagged agent, implementation is Grok's (step 4) and review is astra's (step 5.5), and this section is about the fan-out around them — but you can and should spawn Agent/Task subagents for parallelizable work. The subagent model shares the prompt cache with you and returns a summary — so it's strictly cheaper and faster than dispatching to a separate agent for independent sub-work.
...
- **You are still accountable for the review.** On a lane-flagged agent you adjudicate astra's findings rather than review the code yourself (Kyle 2026-09-25: orchestrate only); elsewhere self-review your diff with the same rigor a dedicated reviewer would — edge cases, regressions, things you'd flag if someone else wrote it. Neither replaces step 5.5 — the internal pass (the Codex lane, or the subagent pass where the lane is not yours) before the first CR push is mandatory.
...
Fast-track runs inline on the orchestrator's main thread and via Agent/Task subagents — it does **not** dispatch to persistent named agents in its own flow. Per plan #280, remote-class persistent agents (today **forge** and, since 782.31, **3dproppipeline** — vaexdev3/vaexserverdev were retired 2026-08-07 per Kyle, deactivated and hidden, to be rebuilt properly when task volume warrants) boot Offline (`AutoSpinDown: true`) and require an explicit `hive_agent_wake` to come up. **Primary agents (vaexdev2, spark, ...) are NOT wake targets** — the server refuses a cold-start wake on them (plan 782.10; a wake on an already-running Primary agent returns a harmless no-op); they come up via their workspace launch script, and you reach a running one with `hive_send_message`.

When fast-track delegates to a sub-skill that does touch a persistent agent, the wake/sleep wrap is that sub-skill's responsibility, not yours. The sub-skills that dispatch to a remote-class agent — **`/deploy-hive`** (wakes forge at step 5, sleeps it at step 9), `deploy-nightwatch-config`, and overwatch's on-demand Hive backup — each carry that wrap themselves.
...
- `deploy-hive` — the Hive platform deploy procedure. Invoke as a sub-step for any plan touching what the deploy ships (step 7 above lists the paths).
