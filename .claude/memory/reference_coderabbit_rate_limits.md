...
- **Pooled per developer identity** (repo-specific observation, follows from limits being per-developer): every agent (overwatch, vaexdev, spark, 3dproppipeline...) authors commits/PRs as kyle-wf, so the entire fleet consumes ONE developer's allowance — that's why the org trips the fair-usage flag despite modest per-repo volume.
