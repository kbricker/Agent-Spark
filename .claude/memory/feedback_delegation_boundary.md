...
2. **Subagent — the DEFAULT for any discrete job** with no context worth keeping afterward: research, validation, adversarial review, verbose output you will not reference again, parallel independent sub-work. Two carve-outs (plan #1046, Kyle 2026-09-25: Grok implements, astra reviews, Claude orchestrates): on an agent flagged `grokLane` in composition.json, ALL implementation on a plan, on a branch you push, goes to the Grok dev lane — never inline and never a Claude subagent, tight loops included (Grok's session continuity is the loop) — with fast-track-plan step 4 naming the two exits to a subagent (containment, lane failure); and on an agent flagged `codexLane`, the adversarial review is the Codex lane, not a subagent. The config-repo flow stays with you, and a planless quick ask stays under Rule 1 (no ticket, least work). Pick the flavor from the harness's own toolbox: fresh-context by default; a **fork** when the job needs your conversation context rather than a long re-brief; **background** for parallelism; a **custom agent type** when it needs its own tools or model; **worktree isolation** when it edits files alongside you.
