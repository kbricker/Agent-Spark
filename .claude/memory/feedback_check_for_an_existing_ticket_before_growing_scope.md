...
Related: [[feedback_no_unrequested_ux]], [[feedback_kyle_sets_start_and_stop]], [[feedback_never_defer_scope]], and the `manage-scope-creep` skill (which covers dev/review-time creep; this one is about creep at *shaping* time, which the skill does not catch).
