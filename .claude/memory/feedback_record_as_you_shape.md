...
- "Contract" means **operator-visible promises only** (Kyle 2026-07-24): what the tool/feature promises its user — inputs required, flags' meaning, refusal conditions (e.g. #643's "requires all six joints"). Internal architecture/topology divergence from plan text (e.g. #645: plan said "capture thread", implementation deliberately runs capture on the main thread) stays findings-only.
