...
- **Who runs the pass gained an OPTION, not a replacement (plans #1014/1014.1, 2026-09-06):** an agent flagged `codexLane: true` in `composition.json` may add a Codex-lane run (`hooks/codex-dispatch.mjs`, fast-track-plan step 5.5) over the same diff as one more lens. It is additive: the Claude subagent pass still happens and still adjudicates, and the pass must ALWAYS happen either way. Not flagged means not yours; an agent whose repos have no CodeRabbit is flagged only with Kyle's explicit yes, and even then the Codex lens stays additive to the Claude pass.
