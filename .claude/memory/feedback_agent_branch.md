...
**How to apply:** When dispatching work to a named agent that owns a long-lived clone (vaexdev, vaexdev2, forge, 3dproppipeline), the first instruction must be `git fetch origin && git checkout <branch> && git pull origin <branch>` in their workspace. Never assume — a long-lived clone sits wherever its last task left it.
