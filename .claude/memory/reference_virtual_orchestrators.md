...
3. **vaexdev2** — **second instance of the same role as vaexdev**, `C:\Projects\vaexdev2\`. Clone `C:\Projects\Unity\VaEx2`. Added 2026-08-04, plan 782.2. Roster record confirmed `virtual` on 2026-08-04; it reads `status: "virtual"` until its first launch, which is the genuine initial state for a virtual agent that has never run — not an error and not an artifact.
