...
**That clause is the only defence the recipient has, and it matters most precisely where they cannot check you. Detectability is a property of the topic, not of the receiver's diligence.**

Both halves of the controlled case happened to one agent on 2026-08-10. forge caught *"you refused to push"* immediately, because that claim was about its own turn, where it held direct evidence. The same agent, in the same session, taking the same care, relayed *"VaEx4 is a dead clone"* straight into plan #865's description as fact — because that one was about a checkout it had no reason to inspect. Same rigour, opposite outcomes; the only variable was whether the subject was something it could see. **There is no level of downstream diligence at which the second one gets caught**, which puts the entire load back on the author.

So when you assert something unverified about a host, a deployment, a plan's state, or another agent's conduct, there is nothing on the far end but your word — most of all when the claim arrives **flattering and declarative**, because praise invites agreement and a briefing's register invites compliance, and nothing anywhere in that exchange rewards checking.
